...
业务：
  ...
  - 顺带回写 馬名（列表页馬名，未命名仔 → 空，供建档时未命名/后续更名的马补名）。
...
_UNNAMED_RE = re.compile(r"^.+の\d{4}$")   # netkeiba 未登録名占位：母名のYYYY（如 サウンドバリアーの2023）


def is_unnamed(name):
    """未命名占位判定：全角下划线 ＿＿＿ 或 netkeiba 未登録名格式「母名のYYYY」。
    返回 True 表示无意义，应存空 ""（不能当作真名覆盖已有馬名）。"""
...
def main():
    ...
    matched, miss, cache, names, total = 0, [], {}, {}, {}
    for nk, r in nk_rows.items():
        key = (norm_mare(r["母名"]), r["生年"])
        h = by_key.get(key)
        if h:
            cache[str(h["id"])] = nk       # 写独立缓存，不碰 basic.json
            names[str(h["id"])] = "" if is_unnamed(r["馬名"]) else r["馬名"]   # 馬名（未命名仔→空）
