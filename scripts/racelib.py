...
def norm_facet(s):
    """facet 归一化：NFKC + 去全半角空白 + toLowerCase（前端零归一化逻辑）。"""
    return re.sub(r"\s+", "", unicodedata.normalize("NFKC", s or "")).lower()


def pedigree_ancestors(pedigree):
    """5 代血统树（父/母两列嵌套数组）→ 全部祖先名去重集合（血统祖先 facet）。
    例: {"父": [[{name:...}], [{name...},{name...}], ...], "母": [...]}。"""
    names = set()
    for side in ("父", "母"):
        for gen in (pedigree or {}).get(side, []) or []:
            for node in gen or []:
...
def build_facet(h):
    """马档案（含 races/pedigree 内嵌时）→ facet 检索索引（M5.2）。
    - search_text: 全字段拼接（唯一模糊层）
    - 强匹配·等值: 馬名/曾用名/性別/生年/登録状態/調教師/馬主/生産牧場/fno
    - 强匹配·集合: 騎手/場名/格/血统祖先/主要場地
    - 布尔: has_races/has_win/has_graded_win/有地方/有海外
    - 人工字段（译名/近况/备考/血统分析）只进 search_text，不做强匹配
    所有字符串值已归一化；原始展示值保留在马档案顶层。"""
    races = h.get("races") or []
    pedigree = h.get("pedigree") or {}

    jockeys = sorted({str(r.get("騎手") or "") for r in races if r.get("騎手")})
    venues = sorted({str(r.get("場名") or "") for r in races if r.get("場名")})
    grades = sorted({str(r.get("格") or "") for r in races if r.get("格")})
    main_venues = sorted({str(r.get("venue_type") or "") for r in races if r.get("venue_type")})
    ancestors = pedigree_ancestors(pedigree)

    results = [r.get("結果") for r in races]
    has_win = 1 in results
    has_graded_win = any(
        r.get("格") in GRADES and r.get("結果") == 1 for r in races)

    # 人工字段只进 search_text（值不稳定、非枚举，见 §3.2.4）
    search_parts = [
        h.get("馬名") or "", " ".join(h.get("曾用名") or []),
        h.get("译名") or "", h.get("近况") or "", h.get("备考") or "",
    ...
    def nf(x):
        return norm_facet(x) if x else ""

    return {
        "search_text": search_text,
        "馬名": nf(h.get("馬名")),
        "曾用名": [nf(x) for x in (h.get("曾用名") or [])],
        "性別": nf(h.get("性別")),
        "生年": nf(h.get("生年")),
        "登録状態": nf(h.get("登録状態")),
        "調教師": nf(h.get("調教師")),
        ...
        "生産牧場": nf(h.get("生産牧場")),
        "fno": nf(h.get("fno")),
        "騎手": [nf(x) for x in jockeys],
        "場名": [nf(x) for x in venues],
        "格": [nf(x) for x in grades],
        "血统祖先": [nf(x) for x in ancestors],
        "主要場地": [nf(x) for x in main_venues],
        "has_races": bool(races),
        "has_win": has_win,
        "has_graded_win": has_graded_win,
        "有地方": "地方" in main_venues,
        "有海外": "海外" in main_venues,
    }


def build_index(horses):
    """facet 列表 → _meta.index 倒排表 {字段: {值: [id…]}}（带计数分面导航，M5.2）。
    仅收录枚举型/集合型字段（等值/集合层），数值与模糊走 per-horse facet。"""
    idx = {}
    facets = {h["id"]: h.get("facet") or {} for h in horses}

    def put(field, value, hid):
        d = idx.setdefault(field, {})
        v = norm_facet(value)
        if v:
            d.setdefault(v, []).append(hid)

    scalar_fields = ("性別", "生年", "登録状態", "調教師", "馬主", "生産牧場", "fno")
    list_fields = ("騎手", "場名", "格", "血统祖先", "主要場地")
    for hid, f in facets.items():
        for field in scalar_fields:
            put(field, f.get(field), hid)
        ...
        if f.get("有海外"):
            put("有海外", "true", hid)
        for field, flag in (("has_races", f.get("has_races")), ("has_win", f.get("has_win")),
                            ("has_graded_win", f.get("has_graded_win"))):
            if flag:
                put(field, "true", hid)
    return idx
