...
# 収得需要本賞金的场次：中央重赏 1/2着 / 地方 Jpn 1/2着（海外不参与収得、非重赏用固定额）
CENTRAL_GRADES = {"GI", "GII", "GIII", "JGI", "JGII", "JGIII"}
JPN_GRADES = {"JpnI", "JpnII", "JpnIII"}


def needs_honsho(rec):
    """该比赛记录是否需要抓本賞金：中央重赏 或 地方 Jpn 重赏 的 1/2着（海外不参与収得）。"""
    if rec.get("結果") not in (1, 2) or not rec.get("race_id"):
        return False
    vt = (rec.get("venue_type") or "").strip()
    g = rec.get("格") or ""
    if vt == "中央":
        return g in CENTRAL_GRADES
    if vt == "地方":
        return g in JPN_GRADES
    return False
...
def coerce_record(row, issues):
    ...
    return {
        ...
        "年齢": age,
        "斤量": _cell(row, "斤量"),
        "枠番": "",   # 台账无枠番/马番字段 → 默认空（netkeiba 记录由成绩页回填）
        "馬番": "",   # 台账无马番字段 → 默认空（netkeiba 记录由成绩页回填）
...
# ── 馬名匹配键 ──
# 去国家后缀：兼容全角/半角括号 + 常见遗漏国家码 + 括号内/尾随空格变体。
# 覆盖 JBIS/netkeiba 可能出现的 (JPN)（含全角（JPN））与海外国家码（含 SGP/UAE/NZL/BR/CL/PE/MX/KOR/TWN）。
_COUNTRY_SUFFIX_RE = re.compile(
    r"[（(]\s*(?:(?:JPN|USA|GB|IRE|NZ|FR|AU|AUS|CAN|GER|ITY|SA|ARG|BRZ|CHI|URU|HK|SGP|UAE|NZL|BR|CL|PE|MX|KOR|TWN))\s*[）)]$")
