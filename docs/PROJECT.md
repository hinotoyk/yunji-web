...
- **本賞金来源**：SP 接口 `race.netkeiba.com/race/result.html?race_id={id}` 直接读 `本賞金:…万円`（付加賞-free，13/13 实测）；回退 = db 域 4/5着 反推（`compute_honsho_prize`，5着/0.10 最干净，4着/0.15 有 +5万 系统偏差）。
...
- **已知差异**：ジーネキング 2%（3100万 vs 设计 3000万，已定案 3100万）；リアライズシリウス 150万（库外不再校验）。
