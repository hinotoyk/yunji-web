...
删空 data/ → ① 建档 build_registry(JBIS) → ② 基础 run_all(并发血统/nk_id/studbook + 详情 + 合并)
         → ③ 竞赛 run_all(详情+判变 → 成绩增量(SP页含格/条件/調教師/本賞金) → 台账 → 合并) → 汇总
```
...
```
run_all --force：详情(全) → 成绩(force 全量重抓，SP页含格/条件/調教師/本賞金) → 台账 → 合并
```
...
python scripts/races/fetch_races.py --id 1,2,3    # 成绩（SP 页含格/条件/調教師/本賞金）
