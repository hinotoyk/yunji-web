...
**遗留（已收口，见 §60）**：当时「通算成績 · RECORD」仍只读 netkeiba 的 `通算成績` 字符串（不含海外台账场），130 因此显示「未出赛」而下方比赛记录明明有 8 场 —— §60 已改为由 `races/*.json` 全量逐场推导，并把 `check_data.py` 的比对左值收口到「`來源=netkeiba` 行数」做同口径对账。
...
## 60. 通算战绩收口 · 改由 races 全量逐场推导 + check_data 同口径对账

**问题**：基本信息「通算成績 · RECORD」带过去是**正则解析 netkeiba 详情页的 `通算成績` 字符串**（`transStats` 转中日文字 + `placeStats` 抠 `[x-x-x-x]`），而那份展示值只覆盖 netkeiba 收录的比赛；海外台账补录的 8 场不在其口径内 → id 130 出现「上方 RECORD 未出赛、下方比赛记录 8 场 2 胜」的同页自相矛盾。根因是**两处真相源**：`stats.json` / `datechart.json` / 比赛记录页早已按逐场全量统计，只有这张卡还在读字符串。

**口径推导（先测出来再定，不猜）**：逐场全量 741 行的 `結果` 取值实测只有 `int / 中止 / 取消 / 除外`（无历史单字），据此定 ——**出走 = 全部记录 − 未出走(取消·除外)**，中止/失格 计入出走并单列未完走，**着外 = 出走 − 1着 − 2着 − 3着（含未完走）**。拿全站 7 匹含未完走/未出走的马逐一回比 netkeiba 字符串：`id1 6行·除外1 → 5戦[1-0-0-4]`、`id21 → 4戦`、`id35 → 7戦`、`id44 → 5戦`、`id47 → 4戦`、`id118 → 9戦`（全部 = 行数−除外，与官方战数一致）、`id50 10行·中止1 → 10戦[0-0-1-9]`（着外含未完走，与官方一致）→ **该口径与 netkeiba 展示值全库等价**，切换后除 130 外 275 匹一个数字都不变（不是近似，是逐匹核过）。

**实现**：① `merge_races.py` 新增 3d `career_from_recs(recs)` → 派生对象 `通算成績_逐场 = {出走,1着,2着,3着,着外,未完走,未出走}`（中文键，同 `収得賞金` 先例），**无条件写入**并 `move_after` 归位到 `通算成績` 下面；`BASIC_TEMPLATE` 同步加该字段。官方 `通算成績` 字符串保持 netkeiba 镜像不动、**降级为纯对账用**；`races_report.md` 新增「逐场推导 ≠ netkeiba 展示值」差异表（实测 **1 匹** = 130，差值即 netkeiba 未收录的海外台账场）。② `profile.html` RECORD 带与三率改读派生对象（`出走` 作分母，与统计页 §34 一致），**删除 `transStats()` 与 `placeStats()`**（无调用点即删，不留死码）。③ `check_data.py` 第 4 项改**同口径对账**：`actual_starts(recs, src)` 加来源过滤，左值从「文件全部行数」改成「`來源=netkeiba` 的出赛行数」—— 原写法拿含台账的总行数比 netkeiba 战数，**台账补一场就能把 netkeiba 抓漏一场抵消掉**（静默漏检）；不等即报（少了=抓漏、多了=重复/异常）并进 `--fix` 补拉清单；另加信息性一行列出有台账补录场次的马，解释「逐场通算战绩为何大于展示值」。

**验证**：`merge_races` 跑完 `通算成績 逐场推导 ≠ netkeiba 展示值 1 匹`、`check_data` **问题 0 项**（新同口径判据在全库 276 匹上零误报）；浏览器实测 130 = `8战2胜 [2-1-0-5]` + 三率 `25%/37.5%/37.5%` + 三赏金格 `—`（海外不带赏金，§57 定稿），回归 1 号 = `5战1胜 [1-0-0-4]` + `20/20/20%`、50 号 = `10战0胜 [0-0-1-9]` + `0/0/10%` 均与切换前逐字一致；控制台零报错；`npm run build` + `python run_update.py --check` 五步全绿。

## 61. 未出走不算通算 · 全站口径审计与日期统计页修复

**规则（用户重申）**：`取消 / 除外` = **未出走**（未出闸），**不计入通算战绩**；`中止 / 失格` = 未完走，进了闸没跑完，**仍算出走**、归着外。§60 定 profile 口径时已按此实现，这次拿规则去扫全站，确认哪些地方合规、哪里漏。

**审计结果**：✅ 已合规 —— `merge_races.career_from_recs`（出走 = 记录 − 未出走）、`stats.html` `starts(b) = b.n − b.exc`、`races.html` 三态筛选（`未出走` 明确标注"不计出走"）、`check_data.actual_starts`（取消/除外 不计出赛）。❌ 违规一处 —— **日期统计页**：`statsOf()` 写的是 `n = runs.length`，把 取消/除外 当成出走计数，且 `placeSlot()` 注释明写「中止/取消/除外/失格 计入第 3 名开外」→ 一并塞进着外段，等于**该页 KPI 出走与通算战绩都虚高**。全库影响面 = 6 条未走行（中央 除外3+取消2、地方 除外1），按月分散在 2025-08 / 2026-02 / 03 / 06 / 08 / 09 各 1 条。
...
## 65. 通算战绩改五段 [出走-1着-2着-3着-着外] · 着外口径全站统一出走

**问题（用户报回）**：「全部的通算战绩都算错了，应该是 总出走-第1名-第2名-第3名-第三名以外（需要是出走的竞赛）」。

**探针先分清两件事**：
- **数值其实没算错**：`merge_races.career_from_recs`（profile 用）与 `datechart.statsOf()` 早已按出走口径、并保证 `1着+2着+3着+着外 == 出走`（§61 修的，有断言钉住）。用户真正看到的是**括号里缺出走数** —— 全站都是 4 段 `[1着-2着-3着-着外]`，出走写在括号外（datechart 是旁边独立的「出走」KPI、profile 是上方 `8战2胜`）。
- **顺带查出一处真不一致**：`stats.html` 的通算战绩 KPI 与倾向矩阵「着外」列算的是 `fin(b) − 前三`（**完赛**口径，扣掉 中止/失格），而同一行的「出走」列是 `starts(b)` → 一行数字加不起来。全库逐场数据里 中止/失格 只有 **1 条**，所以差值恰好 1（基准行 465 → 466），但口径确实与另两页不同。
