...
- `結果`：数字=着顺（**完赛**，1着～着外）；`中止/失格`=**未完赛**（已出闸、无着顺，计入出走）；`取消/除外`=**未出走**（未出闸，不计入出走）——三态口径对齐 netkeiba 出走回数（累計 678 = 677 数字着顺 + 1 中止，取消/除外不计），见 `pages/races.html` 的 `resultCode()` 与 `tests/_verify-races-result.cjs`
