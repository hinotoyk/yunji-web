# 云迹 · 前端交接文档（front · Vite + Tailwind 阶段）

> 本文件记录「云迹」前端重建到当前阶段的全部决策、现状与后续路线，供接手者直接上手。
> 配套文档：`front/README.md`（**架构与编码规范，必须遵守**）。
...
| 项 | 决策 |
|---|---|
| **构建工具** | **Vite 5 多页构建**（`front/` 为源，`npm run build` → 产物到项目根 `dist/`） |
...
> ⚠ 详细编码规范见 `front/README.md`「架构与编码规范（必须遵守）」。
...
| **工作流** | 正式源码在 `front/`（Vite + Tailwind）；修改 → `npm run build` → 产物 `dist/`；探索/候选版放 `tests/`，废案放 `tests/_trash/` |
...
├─ package.json
├─ README.md               # ★ 架构与编码规范（必须遵守）
├─ HANDOFF.md              # 本文档
├─ pages/                  # ★ 功能子页面（被 index.html iframe 内嵌）
│  ├─ theme.css            # ★ Tailwind 输入：@tailwind 指令 + @layer components 通用组件类 + 字体
│  ├─ profile.html         # 基本信息    ✅ 已完成（三段式+内嵌比赛+内嵌血统）
│  ├─ pedigree.html        # 血统图      ✅ 已完成（独立/内嵌双模式；profile 内嵌展示，侧边栏暂不挂入口）
│  ├─ races.html           # 比赛记录    ✅ 已完成（KPI+逐场表，独立/嵌入两模式）
│  ├─ stats.html           # 统计总览    （占位）
│  ├─ datechart.html       # 日期图      （占位）
│  └─ timeline.html        # 时间线      （占位）
├─ public/                 # ★ 原样拷贝到 dist/ 根的共享 JS（页面用 ../xxx.js 引用）
...
- `結果`：数字=着顺（**完赛**，1着～着外）；`中止/失格`=**未完赛**（已出闸、无着顺，计入出走）；`取消/除外`=**未出走**（未出闸，不计入出走）——三态口径对齐 netkeiba 出走回数（累計 678 = 677 数字着顺 + 1 中止，取消/除外不计），见 `pages/races.html` 的 `resultCode()` 与 `scripts/races/verify_result.cjs`
...
2. **血统图页** `pages/pedigree.html`：选马器 + 简约 2 代 / 完整 5 代（点击弹窗）+ FNo/クロス — ✅ **已完成**（独立 / 被 profile 内嵌两模式，与 races.html 同套路；侧边栏暂不挂入口）
...
- **三段式基本信息页** `profile.html`：
  ...
  - **血统区块**：位于信息头下、KPI 上，内嵌 `pedigree.html?embed=1`（选中马自动联动），展示**简约版**（完整版同款树状网格只取前 2 代，父上母下，行高压到 2/3 适配容器）；**点击弹窗**看完整 5 代。
...
- **样式一律用 Tailwind utility 类**（编译期）；共享组件类在 `pages/theme.css` 的 `@layer components` 里用 `@apply` 封装；页面 `<link>` 引 `theme.css`（Tailwind 编译输入，产物在 `dist/assets/theme-*.css`）。**不要散落写死 hex / 原生 CSS**（指定色块如名字色块除外）。
- **JS 动态类必须能被扫描**：`selector.js` 等在 JS 里拼接 Tailwind 类，其所在目录必须在 `tailwind.config.js` 的 `content`（含 `./pages/**/*.js` 与 `./public/**/*.js`）；改完 `npm run build` 后核对产物 CSS 含新类。
- **主题 token**：颜色/字体在 `tailwind.config.js` 映射（primary 青绿、shadcn Neutral 灰阶、Noto Sans SC + Geist）。CSS 变量语义色对应 Tailwind 命名色（`text-primary` / `text-chart-2` 等）。
- **数据语义色**（全站一致）：
  - 1着/胜 = `primary`（青绿）
  - 2着 = `chart-2`（青）
  - 3着 = `chart-3`（橙）
...
- **共享类**（`theme.css` `@layer components`）：`.card`、`.kpis/.kpi(.v/.hi/.k)`、`.tblwrap`、`table`（`thead/tbody`、`td.win/.num/.sub`）、`.sec-title`、`.badge`、`.btn(.ghost)`、`.fieldgrid/.field`、`.hint-empty`
