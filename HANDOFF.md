...
> `Z:\IdeaProjects\yunji-web\request-path.html`（单匹马请求链路节点图——你现在想重新梳理的就是它）
...
request-path.html        # ★ 唯一请求·数据流路径图（基础+竞赛 3 视图，浏览器打开）
```
...
data/basic.json(id, nk_id)
 ...
 ② fetch_races.py       db.netkeiba.com/horse/result/{nk_id}/（EUC-JP）
    ...
    每条新增记录再抓一次 SP 比赛结果页（race/nar.netkeiba.com，同场多马共享页缓存）一次回填：
      ...
      → 本賞金（中央重赏 1/2着 / 地方 Jpn 1/2着，页内阶梯取该马着順那档）
...
| 比赛增量 | `--races` | 详情更新+判变 → 成绩增量（SP 页含格/条件/調教師/本賞金）→ 台账海外 → 合并 |
| 定向更新 | `--horse 1,2,3` | 只处理指定 id（详情+成绩+合并） |
...
### D.3 部署（GitHub Pages 前端）

- `.github/workflows/deploy.yml` 在 push 到 main 时：`setup-node`（Node 20）→ `front/` 下 `npm ci && npm run build`（产物输出到仓库根 `dist/`）→ 把**仓库根 `./`**（含 `dist/` + `data/`）发布为 Pages 站点根。
- 访问路径 = `https://<user>.github.io/<repo>/dist/index.html`，`data/` 在站点根 `/data`，与本地 `:8090` 结构一致；**`dist/` 保持 gitignore**（CI 现场构建，不入库）。
- 前置配置：Settings → Pages → Source 选 **GitHub Actions**；Settings → Actions → Workflow permissions 开启读写（供 update-data.yml 自动提交 data/）。
