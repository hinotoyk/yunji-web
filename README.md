...
├── request-path.html          # ★ 唯一「请求·数据流」路径图（基础+竞赛 3 视图）
├── README.md                  # 本文件（总览）
...
3. **两条线性单链**（详见 request-path.html）：
   ...
   - 竞赛：详情更新+判变 → 成绩增量（SP 页含格/条件/調教師/本賞金）→ 台账海外 → merge_races
...
| 比赛增量 | `--races` | 详情更新+判变 → 成绩增量(判变∪缺失∪无文件，SP 页含格/条件/調教師/本賞金) → 台账海外 → 合并 | 赛后日常 |
| 定向更新 | `--horse 1,2,3` | 只处理指定 id 的马（详情+成绩+合并） | 手动补单匹 |
...
> 部署（GitHub Pages 前端）：`.github/workflows/deploy.yml` 在 push 到 main 时自动 `npm run build` 生成 `dist/`，并把**仓库根 `./`**（含 `dist/` + `data/`）发布为站点根，访问 `https://<user>.github.io/<repo>/dist/index.html`；`dist/` 保持 gitignore（CI 现场构建，不入库）。前置：Settings → Pages → Source 选 **GitHub Actions**。
